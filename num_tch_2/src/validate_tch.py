...
def main():
    ...
    if len(sys.argv) != 2:
        print("Current teacher number: number between 1 and num_tch is required")
        return
    ...
    test_df = torch.load(os.path.join(processed_dir, f"test_df_{cur_tch}"))

    model_save_path = os.path.join(model_dir, f"teacher_{cur_tch}.pth")
    
    res = run_val(test_loader, test_df, app_name, model_save_path)
    
    file_path = f"res/teacher_{cur_tch}.json"
    with open(file_path, "w") as f:
        json.dump(str(res), f)

    print(f"Done: results saved at: res/teacher_{cur_tch}.json")
