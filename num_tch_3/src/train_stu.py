...
def train(ep, train_loader, model_save_path):
    ...
    for batch_idx, (data, target) in enumerate(train_loader):#d,t: (torch.Size([64, 1, 784]),64)
        ...
        loss = alpha * student_loss + (1 - alpha) * (distillation_loss_1 + distillation_loss_2 + distillation_loss_3)
...
def run_epoch(epochs, early_stop, loading, tch_1_model_save_path, tch_2_model_save_path, tch_3_model_save_path, model_save_path, train_loader, test_loader, live):
...
def main():
    ...
    params = dvc.api.params_show("e.yaml")
    ...
    with Live(dir="res", resume=True) as live:
        live.step = 1
        run_epoch(epochs, early_stop, loading, tch_1_model_save_path, tch_2_model_save_path, tch_3_model_save_path, model_save_path, train_loader, test_loader, live)
